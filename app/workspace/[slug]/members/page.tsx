...
const ROLE_STYLES: Record<string, string> = {
  owner:
    "bg-[color-mix(in_oklab,var(--warning)_14%,transparent)] text-[var(--warning)] border-[color-mix(in_oklab,var(--warning)_30%,transparent)]",
  admin:
    "bg-[color-mix(in_oklab,var(--primary)_14%,transparent)] text-[var(--primary)] border-[color-mix(in_oklab,var(--primary)_30%,transparent)]",
  member:
    "bg-[color-mix(in_oklab,var(--success)_14%,transparent)] text-[var(--success)] border-[color-mix(in_oklab,var(--success)_30%,transparent)]",
  viewer:
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4 bg-[var(--bg)]">
        <p className="text-[var(--danger)]">Failed to load members.</p>
        <Button
          variant="outline"
          onClick={() =>
            queryClient.invalidateQueries({ queryKey: ["members", slug] })
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      <div className="p-6 md:p-8 space-y-6 max-w-5xl mx-auto">
        <FadeIn>
          <div>
            <h1 className="text-display text-3xl font-bold tracking-[-0.02em] text-[var(--fg)]">Members</h1>
        ...
        {isLoading ? (
          ...
          <Card className="bg-[var(--surface)] border-[var(--border)]">
            <CardHeader>
              <CardTitle className="tracking-[-0.01em]">Pending Invites</CardTitle>
              <CardDescription>Waiting for these people to join</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {data.invites.map((invite) => (
                  <div
                    key={invite.id}
                    className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] p-3"
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[color-mix(in_oklab,var(--primary)_10%,transparent)]">
                        <Mail className="h-4 w-4 text-[var(--primary)]" />
                      </div>
                      <div>
                        <p className="text-sm font-medium text-[var(--fg)]">{invite.email}</p>
                        <div className="flex items-center gap-2 text-xs text-[var(--fg-subtle)]">
                          <span>Invited {formatDate(invite.createdAt)}</span>
                          <Badge
                            variant="outline"
                            className={cn("capitalize text-[10px]", ROLE_STYLES[invite.role])}
                          >
                            {invite.role}
                          </Badge>
                        </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[var(--border)] bg-[var(--surface-muted)]">
                      <th className="px-4 py-3 text-left font-medium text-[var(--fg-muted)]">Member</th>
                      <th className="px-4 py-3 text-left font-medium text-[var(--fg-muted)]">Role</th>
                      <th className="px-4 py-3 text-left font-medium text-[var(--fg-muted)]">Joined</th>
                      {isAdmin && (
                    ...
                    {data.members.map((member) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <Badge
                            variant="outline"
                            className={cn("capitalize", ROLE_STYLES[member.role])}
                          >
                            {member.role}
                          </Badge>
                        </td>
